...
if (AddProduct(categoryId: 6,
  productName: "Vojtas cake", price: 500M))
{
    WriteLine("Add product successful.");
}

//if (IncreaseProductPrice(
//  productNameStartsWith: "Jack", amount: 20M))
//{
...
//int deleted = DeleteProducts(productNameStartsWith: "Tofu");
...
static void CategoryById(int inputCagegoryId)
{
    using (Northwind db = new())
    {
        IQueryable<Category>? categories = db.Categories?.Where(c => c.CategoryId == inputCagegoryId);
        foreach (Category c in categories)
        {
            Write("CategoryID: {0}, CategoryName: {1}", c.CategoryId, c.CategoryName);
        }
    }
}
